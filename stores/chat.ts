...
export const useChatStore = defineStore("chat", {
   state: () => ({
      ...
      lastMessageTimestamp: useLocalStorage(
   ...
   getters: {
      whitelistedChats: (state) => {
         ...
         return state.chats.filter((chat) => {
            ...
            if (
               chat.lastMessage?.isSent &&
               whitelist.includes(chat.lastMessage.recipientPubkey)
            )
               return true;
            return false;
         });
   ...
   actions: {
      ...
      async init() {
         ...
         const storedChats = await db.chats
      ...
      async fetchChats() {
         ...
         if (!ndk || !nostrStore.pubkey || !nostrStore.authenticated) {
            console.log("Nostr not ready:", {
               ndk: !!ndk,
               pubkey: !!nostrStore.pubkey,
               authenticated: nostrStore.authenticated,
            });
            return;
         }
         ...
         try {
            ...
            const sub = ndk.subscribe(filter as unknown as NDKFilter, {
               closeOnEose: false,
            });
               ...
               await this.processMessageInQueue(async () => {
                  try {
                     ...
                     if (
                        ...
                        unwrappedMessage.created_at > this.lastMessageTimestamp
                     ) {
                     ...
                     const message: ChatMessage = {
                        ...
                        userProfile: unwrappedMessage.user?.profile
                           ? JSON.parse(
                                JSON.stringify(unwrappedMessage.user.profile),
                             )
                           : undefined,
      ...
      async addMessage(message: ChatMessage) {
         ...
         const chatPubkey = message.isSent
         ...
         if (!chat) {
            ...
            if (ndk) {
               try {
                  ...
                  console.error(
                     "Error fetching user profile for new chat:",
                     error,
                  );
               }
            }
         ...
         if (!chat.messages.find((m) => m.id === message.id)) {
            ...
            if (
               !chat.lastMessage ||
               message.created_at > chat.lastMessage.created_at
            ) {
