...
test('login with NOSTR recovery phrase', async ({ page }, testInfo) => {
    ...
    const words = [
        'ring', 'fine', 'foot', 'behind', 'blouse', 'ten',
        'brown', 'apart', 'tuition', 'toddler', 'fabric', 'emotion'
    ];

    // Take screenshot of recovery phrase form
    const loginScreenshot = await page.screenshot();
    await testInfo.attach('recovery-phrase-form', { body: loginScreenshot, contentType: 'image/png' });
    ...
    // Verify the pubkey
    const expectedPubkey = '2255c59bcb5b38c05f45815fc6dfbbfe6f02e714c47b68d5e1446157002f98e4'
    ...
    await testInfo.attach('nostr-keys-view', { body: keysScreenshot, contentType: 'image/png' });
}) 
