...
test("basic new user flow - create nostr account", async ({
   page,
}, testInfo) => {
   // Go to the home page
   await page.goto("/");
   ...
   await page.waitForFunction(() => {
      return (
         window.localStorage.getItem("nostr-store") !== null &&
   ...
   await page.waitForFunction(
      () => {
         ...
         return map && !loading && window.getComputedStyle(map).opacity === "1";
      },
      { timeout: 30000 },
   );
   ...
   await testInfo.attach("map-view", {
      body: mapScreenshot,
      contentType: "image/png",
   });

   // Click View Properties button (with dynamic number)
   const viewPropertiesButton = page.getByRole("button", {
      name: /View \d+ properties/,
   });
   ...
   await testInfo.attach("property-details-view", {
...
test("navigate through settings menu", async ({ page }, testInfo) => {
   ...
   for (const { name, expectedUrl } of menuTests) {
      ...
      await testInfo.attach(`menu-${name.toLowerCase()}-view`, {
         body: screenshot,
         contentType: "image/png",
      });
