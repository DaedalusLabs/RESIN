   ...
   <section class="flex h-full flex-col overflow-hidden">
      ...
      >
         ...
         <div class="flex items-center justify-between">
            <button
               class="flex items-center gap-2 rounded-lg bg-resin-500 px-4 py-2 text-sm text-white hover:bg-resin-600 disabled:opacity-50"
            ...
            <div v-if="isLoadingHistory" class="ml-4 flex flex-col gap-2">
               <div class="flex items-center gap-2">
                  ...
                  <div
                     class="h-2 w-48 overflow-hidden rounded-full bg-gray-200"
                  >
                     <div
                        class="h-full bg-resin-500 transition-all duration-300"
                        :style="{
                           width: `${(processedEvents / totalEvents) * 100}%`,
                        }"
                     ></div>
                  </div>
               </div>
               <div v-if="decryptionPhase" class="flex items-center gap-2">
                  ...
                  <div class="text-sm text-gray-600">
                     {{ decryptedEvents }}/{{ totalDecryptEvents }}
                  </div>
                  <div
                     class="h-2 w-48 overflow-hidden rounded-full bg-gray-200"
                  >
                     <div
                        class="h-full bg-resin-500 transition-all duration-300"
                        :style="{
                           width: `${(decryptedEvents / totalDecryptEvents) * 100}%`,
                        }"
                     ></div>
                  </div>
               </div>
         ...
         <div class="text-center">
            <div class="mb-4">
               <img
                  ...
                  class="mx-auto h-12 w-12 animate-pulse"
                  alt="Loading"
               />
            </div>
            ...
            <template v-if="selectedChat">
               ...
               <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4">
                  <FlowbiteChatBubble
                     ...
                     :message="msg.content"
                     :name="
                        formatDisplayName(msg.userProfile?.name, msg.pubkey)
                     "
                  ...
                  >
                     <template #below-message>
                        <div v-if="isPropertyReference(msg)" class="mb-4 mt-2">
                           <Suspense>
                              <PropertyCard
                                 v-if="msg.property"
   ...
   () => {
      if (selectedChat.value) {
         const updatedChat = chatStore.chats.find(
            (c) => c.pubkey === selectedChat.value?.pubkey,
         );
         if (updatedChat) {
   ...
   try {
      ...
      for (const event of eventArray) {
         try {
            ...
            // Cast through unknown to bypass type checking since we know the method exists
            const unwrappedMessage = await (
               nostrStore as unknown as {
                  unwrapMessage: (event: NDKEvent) => Promise<UnwrappedMessage>;
               }
            ).unwrapMessage(event);
            ...
            const message = {
               ...
               userProfile: unwrappedMessage.user?.profile
                  ? JSON.parse(JSON.stringify(unwrappedMessage.user.profile))
                  : undefined,
...
watch(
   ...
   async (messages?: MessageWithProperty[]) => {
      if (messages) {
...
function hideChat(pubkey: string) {
   ...
   localStorage.setItem(
      "hidden-chats",
      JSON.stringify(Array.from(hiddenChats.value)),
   );
