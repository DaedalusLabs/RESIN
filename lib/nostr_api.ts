import NDK, { NDKEvent, NDKFilter, NDKSubscriptionOptions, NDKUser } from '@nostr-dev-kit/ndk';
...
export class NostrAPI {
    ...
    async request<T>(targetPubkey: string, method: commands, params?: Record<string, unknown>): Promise<T> {
        if (!this.ndk.signer) {
        ...
        let published = await event.publish();
    ...
    async handleRequests(handler: (method: commands, params?: Record<string, unknown>) => Promise<unknown>): Promise<void> {
        if (!this.ndk.signer) {
        ...
        subscription.on('event', async (event: NDKEvent) => {
            console.log('Received event:', event.id, 'via', event.onRelays.map(r => r.url));
            try {
                if (!this.ndk.signer) throw new Error('No signer available');
