      ...
      <div class="flex items-center justify-between gap-2">
         <form class="w-full max-w-full flex-grow" @submit.prevent>
            ...
            <div class="relative">
               ...
               <section class="border-transparent focus:outline-border-pirate-700 block w-full rounded-2xl border border-pirate-700 bg-black p-4 pr-7 ps-10 outline-none focus:border-pirate-700 focus:ring-0 min-h-14">

               <ais-autocomplete>
                  <template #default="{ currentRefinement, indices, refine }">
                     <input id="default-search" :value="currentRefinement" type="search"
                        class="border-none text-sm text-gray-900 w-full text-pirate-50 bg-transparent outline-none focus:outline-none focus:ring-0"
                        :placeholder="$t('map.search.placeholder')" required autocomplete="off" @input="(e) => {
                              refine(e.currentTarget.value);
                              isDropdownOpen = true;
                           }
                           " />
                     <button v-if="currentRefinement" type="button"
                        class="absolute inset-y-0 end-0 flex items-center pe-3" :aria-label="$t('map.search.clear')"
                        @click="refine('')">
