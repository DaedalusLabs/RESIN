<template>
   <FlowbiteDrawer
      :is-open="showDrawer"
      class="p-4"
      :hide-title="verificationStarted"
      :close-on-backdrop-click="false"
      @close="handleCloseDrawer"
   >
      <template #title>
         <h3
            v-show="!verificationStarted"
            class="text-xl font-semibold text-pirate-950"
         >
      ...
      <template #content>
         ...
         <section v-show="!verificationStarted" class="text-center">
            <p>
               You will be redirected to our identity provider to verify your
               identity.
            </p>
            <FlowbiteButton
               text="Continue"
...
const getAccessToken = async () => {
   ...
   const response = await fetch(
      `${runtimeConfig.public.BACKEND_ENDPOINT}/verification/token`,
      {
         method: "POST",
         headers: {
